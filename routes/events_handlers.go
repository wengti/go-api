...
func registerEvent(context *gin.Context) {
	...
	err = db.RegisterEvent(eventId, int64(fltUserId))
	if err != nil {
		context.JSON(http.StatusInternalServerError, gin.H{"message": err.Error()})
		return
	}

	context.JSON(http.StatusOK, gin.H{"message": fmt.Sprintf("You have succefully registered for event with id %v.", eventId)})
}

func deregisterEvent(context *gin.Context) {
	// Get event ID from path and convert to integer
	eventId, err := strconv.ParseInt(context.Param("id"), 10, 64)
	if err != nil {
		context.JSON(http.StatusBadRequest, gin.H{"message": err.Error()})
	...
	err = db.DeregisterEvent(eventId, int64(fltUserId))
	if err != nil {
		context.JSON(http.StatusInternalServerError, gin.H{"message": err.Error()})
		return
	}

	context.JSON(http.StatusOK, gin.H{"message": fmt.Sprintf("You have succefully deregistered for event with id %v.", eventId)})
}
